...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        self.conv = nn.Conv2d(channels,num_filters,kernel_size,stride_conv,int(padding_conv))
        self.conv.weight.data.mul_(weight_scale)

        heightAfterConv = 1 + (height - kernel_size + 2*padding_conv) / stride_conv
        widthAfterConv = 1 + (width - kernel_size + 2*padding_conv) / stride_conv

        self.pool_size = pool
        self.pool_stride = stride_pool
        self.maxPoolPadding = 1

        heightAfterPool = floor(1 + (heightAfterConv - self.pool_size + 2*self.maxPoolPadding) / self.pool_stride)
        widthAfterPool = floor(1 + (widthAfterConv - self.pool_size + 2*self.maxPoolPadding) / self.pool_stride)

        self.fc1 = nn.Linear(num_filters*heightAfterPool*widthAfterPool, hidden_dim)
        self.fc1.weight.data.mul_(weight_scale)
        self.fc2 = nn.Linear(hidden_dim, num_classes)
        self.fc2.weight.data.mul_(weight_scale)
    ...
    def forward(self, x):
        ...
        x = F.max_pool2d(F.relu(self.conv(x)),self.pool_size,self.pool_stride,self.maxPoolPadding)
